...
async function runTests() {
  ...
  const res1 = await AIService.processBatch(standardLeads, 0);
  ...
  const res2 = await AIService.processBatch(messyLeads, 0);
  ...
  const res3 = await AIService.processBatch(invalidLeads, 0);
